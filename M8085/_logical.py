...
class Logical:
    ...
    def __rlc(self):
        accumulator_value = encode(decode(self.__register['A']),'bin')[2:].zfill(8)
    ...
    def __ana(self, arg: list):
        r = arg[0]
        if r == 'M':
            self.__register['A'] = encode(decode(self.__register['A']) & decode(self.__memory_address[self.__rp()]))
        else:
